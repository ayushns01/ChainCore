...
#### 1. Network Node (`src/nodes/network_node.py`)
...
#### 2. Mining Client (`src/clients/mining_client.py`)
...
3. **Optional - Set up PostgreSQL:**
   ...
   # Configure connection in src/data/config.py
   ```
...
3. **Start mining:**
   ...
   ```bash
   python src/clients/mining_client.py --wallet 1A1zP1eP5QGefi2DMPTfTL5SLmv7DivfNa --node http://localhost:5001
   ```

4. **Monitor the blockchain:**
   ```bash
   python src/monitoring/blockchain_monitor.py monitor
   ```
