...
python wallet_client.py create --wallet miner1.json
python wallet_client.py create --wallet miner2.json  
python wallet_client.py create --wallet miner3.json
```

### **3. Start Mining Clients**
```bash
# Terminal 4 - Miner 1
python mining_client.py --wallet MINER1_ADDRESS --node http://localhost:5001

# Terminal 5 - Miner 2
python mining_client.py --wallet MINER2_ADDRESS --node http://localhost:5002

# Terminal 6 - Miner 3  
python mining_client.py --wallet MINER3_ADDRESS --node http://localhost:5003
```

### **4. Send Transactions**
```bash
# Send transactions using wallet client
python wallet_client.py send --wallet miner1.json --to RECIPIENT_ADDRESS --amount 25
```
...
```bash
python network_node.py --help
python network_node.py --node-id core1 --api-port 5001 --p2p-port 8001
python network_node.py --no-discover --debug --quiet
```

### **Mining Client Commands**  
```bash
python mining_client.py --help
python mining_client.py --wallet ADDRESS --node http://localhost:5001
python mining_client.py --wallet ADDRESS --stats --quiet
```

### **Wallet Client Commands**
```bash  
python wallet_client.py --help
python wallet_client.py create --wallet miner.json
python wallet_client.py send --wallet miner.json --to ADDRESS --amount 50
```
...
python test_competitive_mining.py
...
for i in {1..5}; do
  python network_node.py --node-id core$i --api-port $((5000+i)) &
done

# Start multiple mining clients
for i in {1..5}; do
  python mining_client.py --wallet ADDRESS_$i --node http://localhost:$((5000+i)) &
done
```
